...
//       price: r.price,
//       unit: r.unit,
//       hasStock: r.hasStock || false, // NEW
//       stockQty: r.stockQty || 0, // NEW
//     }));
...
//   // Use the picked date, or default to today
...
//         html: `<div class="dash-act-row" onclick="showPosBill('${b.id}')"><div><div class="dash-type">Walk-in POS</div><div class="dash-desc">Receipt ${b.receiptNo}</div></div><div class="dash-amt-in">+${inr(b.total)}</div></div>`,
//       });
//     });

//   // 2. Process Ledger Bills
//   entries
...
//         html: `<div class="dash-act-row" onclick="showBill('${e.id}')"><div><div class="dash-type" style="color: var(--accent);">Ledger Bill</div><div class="dash-desc">${e.name} (${e.billNo})</div></div><div class="dash-amt-in" style="color: var(--tx-2); font-size: 13px;">Advance: +${inr(e.adv)}</div></div>`,
//       });
//     });

//   // 3. Process Staff Khata
//   staff.forEach((s) => {
//     (s.khata || [])
...
//           activities.push({
//             time: Date.now(),
//             html: `<div class="dash-act-row" style="background: #fffafa;" onclick="openKhata('${s.id}')"><div><div class="dash-type" style="color: var(--red);">Staff Payout</div><div class="dash-desc">${s.name} - Advance</div></div><div class="dash-amt-out">-${inr(k.amount)}</div></div>`,
//           });
//         }
//       });
//   });
...
//       return `
//       <div class="icard" style="padding-bottom: 16px; border-bottom: 3px solid var(--accent); display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; height: 100%; ${isOut ? "opacity: 0.5; filter: grayscale(100%); pointer-events: none;" : ""}" onclick="addPosItem('${it.id}')">
...
//     alert(`Cannot add! Only ${item.stockQty} ${item.unit} available in stock.`);
//     return;
//   }
...
//     closeOv("ov-item");
//     await new Promise((resolve) => setTimeout(resolve, 100)); // Blur fix
//     await loadData(true);
//   } catch (err) {
//     console.error("Save Error:", err);
//   }
